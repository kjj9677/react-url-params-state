...
export function useQueryParams<T extends QueryparamConfig>(
  ...
  QueryParamResult<T>,
  (patch: Partial<QueryParamResult<T>>, mode?: HistoryMode) => void
] {
  const {
    sortKeys = false,
  ...
  const getParams = useCallback((): QueryParamResult<T> => {
    ...
    const urlNeedsUpdate = { value: false };
    const updatedParams: Partial<QueryParamResult<T>> = {};
    ...
    for (const key of typedKeys(config)) {
      ...
      if (type === "string[]") {
        ...
        out[key] = (parsed ??
          defaultValues[key]) as QueryParamResult<T>[typeof key];
      } else {
        const parsed = v.parseSingle(
          sp.get(keyStr),
          type as SingleType,
          customParser as
            | ((value: string) => string | number | boolean | Date | undefined)
            | undefined,
          onError,
          keyStr
        );
      ...
      if (
        validator &&
        out[key] != null &&
        !validator(out[key] as QueryparamTypeMap[T[typeof key]])
      ) {
        if (onError) {
          onError(
            new Error(`Validation failed for ${keyStr}`),
            keyStr,
            out[key]
          );
        }

        if (validationFailureMode === "keep") {
          // Keep the invalid value in state (original behavior)
        } else if (validationFailureMode === "remove") {
          // Remove from URL and use undefined in state
          out[key] = undefined as QueryParamResult<T>[typeof key];
          updatedParams[key] = undefined as QueryParamResult<T>[typeof key];
          urlNeedsUpdate.value = true;
        } else if (validationFailureMode === "useDefault") {
          // Remove from URL and use default value
          out[key] = defaultValues[key] as QueryParamResult<T>[typeof key];
          if (defaultValues[key] != null) {
            updatedParams[key] = defaultValues[
              key
            ] as QueryParamResult<T>[typeof key];
          } else {
            updatedParams[key] = undefined as QueryParamResult<T>[typeof key];
    ...
    // Update URL if validation failures require it
    if (urlNeedsUpdate.value && typeof window !== "undefined") {
      setTimeout(() => {
        const currentSp = getSearchParams();
        const before = sortKeys
          ? toSortedQueryString(currentSp)
          : currentSp.toString();

        for (const key of typedKeys(updatedParams)) {
          const type = config[key];
          const nextVal = updatedParams[key];
          const keyStr = String(key);
          const customSerializer = customSerializers[key]?.serialize;

          if (type === "string[]") {
            currentSp.delete(keyStr);
            if (isStringArray(nextVal) && nextVal.length > 0) {
              nextVal.forEach((value) => currentSp.append(keyStr, value));
            }
          } else if (type === "object") {
            const serialized = v.serializeObject(nextVal);
            if (serialized != null) {
              currentSp.set(keyStr, serialized);
            } else {
              currentSp.delete(keyStr);
            }
          } else {
        ...
        if (after !== before) {
          ...
          applyUrl(newUrl, defaultHistory);
        }
      }, 0);
    }
  ...
  const setParams = useCallback((next: QueryParamResult<T>) => {
    _setParams((prev) => {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log("[react-url-params-state] State updated:", {
          prev,
          next,
          merged,
        });
  ...
  useEffect(() => {
    ...
    const syncDefaultsToUrl = () => {
      ...
      for (const key of typedKeys(defaultValues)) {
        ...
        if (defaultValue != null && !sp.has(keyStr)) {
          urlNeedsUpdate = true;

          if (type === "string[]") {
            if (isStringArray(defaultValue) && defaultValue.length > 0) {
              defaultValue.forEach((value) => sp.append(keyStr, value));
            }
          } else if (type === "object") {
            const serialized = v.serializeObject(defaultValue);
            if (serialized != null) {
              sp.set(keyStr, serialized);
            }
          } else {
    ...
    const handle = () => {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          "[react-url-params-state] URL changed, updating state:",
          newParams
        );
      }
      setParams(newParams);
